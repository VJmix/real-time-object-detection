...
api_url = 'https://api.thingspeak.com/update?api_key=O4M2V4IWAQAQZ9FZ'
counter = 0
...
while True:
	...
	for i in np.arange(0, detections.shape[2]):
		...
		if confidence > args["confidence"]:
			...
			PARAMS = {'field1': detected_objects[-1].split(':')[0]}
			if(counter % 500 == 0):
				req_res = requests.get(url = api_url, params = PARAMS)
				counter = 0
			counter = counter + 1
